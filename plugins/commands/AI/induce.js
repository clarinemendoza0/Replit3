...
async function onCall ({ message, args }) {
      ...
      if (!args[0]) {
    message.reply('𝗜𝗡𝗧𝗥𝗢𝗗𝗨𝗖𝗧𝗜𝗢𝗡\n\nInduce is a command that can be used to motivate yourself or others. It takes a number between 1 and 100 as input.\n\nTo use Induce, simply type [Induce [number]]. For example, [Induce 60].',);
    return;
  }

  if (isNaN(number) || number < 1 || number > 100) {
    message.reply('Invalid input. Please provide a number between 1 and 100.',);
    return;
  }
  ...
  try {
    ...
    if (data) {
      ...
      const messages = `✨ 𝗧𝗜𝗧𝗟𝗘: ${formattedTitle}\n\n – ${formattedContent}`;
      message.reply(messages);
    } else {
      message.reply('Entry not found. Please provide a valid number.');
    }
  } catch (error) {
    console.error(error);
