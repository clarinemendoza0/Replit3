...
async function onCall({ message, args, getLang, userPermissions }) {
  ...
  const MAX_LOAN_AMOUNT = 5000000000; //
  ...
  // Handle 'deposit' subcommand
    ...
    if (!bankData[targetID]) {
      ...
      const userMoney = parseFloat(await Users.getMoney(targetID));
  ...
  // Handle 'transfer' subcommand
    ...
    const transferAmountArg = args[2];

    if (!recipientName || !transferAmountArg || isNaN(transferAmountArg) || parseFloat(transferAmountArg) <= 0) {
      message.reply(getLang("error"));
    } else {
      const transferAmountArgLower = transferAmountArg.toLowerCase();

      // Check if the sender has an account and sufficient balance
      if (!bankData[targetID]) {
        message.reply(getLang("no.account"));
      } else if (transferAmountArgLower === 'all') {
        // Check if the sender has a non-zero balance
        if (bankData[targetID].coin <= 0) {
          message.reply(getLang("no.money"));
        } else {
          // Transfer all funds to the recipient

          // Find the recipient's account by name or bank name
          let recipientID = null;
          for (const userID in bankData) {
            if (
              bankData[userID].name === recipientName || // Check by user name
              bankData[userID].bankName === recipientName // Check by bank name (if you have it in your data structure)
            ) {
              recipientID = userID;
              break;
            }
          ...
          if (recipientID) {
            const senderBalance = bankData[targetID].coin;

            // Deduct the entire balance from the sender's account
            bankData[targetID].coin = 0;

            // Add the entire balance to the recipient's account
            bankData[recipientID].coin += senderBalance;

            try {
              // Update the JSON file
              await fs.writeFile(PATH, JSON.stringify(bankData, null, 2), 'utf-8');
              const formattedTransferAmount = senderBalance.toLocaleString(undefined, {
                maximumFractionDigits: 2,
              });
              message.reply(`【 ℹ 】➜ Transferred ₱${formattedTransferAmount} to ${recipientName}.`);
            } catch (error) {
              console.error('Error writing to JSON file:', error);
              message.reply(getLang("error"));
            }
            ...
            message.reply(`【 ℹ 】➜ Recipient "${recipientName}" not found.`);
          }
        }
      } else {
        const transferAmount = parseFloat(transferAmountArg);

        // Check if the sender has sufficient balance
        if (bankData[targetID].coin < transferAmount) {
          message.reply(getLang("no.money"));
        } else {
          // Find the recipient's account by name or bank name
          let recipientID = null;
          ...
          if (recipientID) {
            ...
            bankData[targetID].coin -= transferAmount;

            // Add the amount to the recipient's account
            bankData[recipientID].coin += transferAmount;

            try {
              // Update the JSON file
              await fs.writeFile(PATH, JSON.stringify(bankData, null, 2), 'utf-8');
              const formattedTransferAmount = transferAmount.toLocaleString(undefined, {
                maximumFractionDigits: 2,
              });
              message.reply(`【 ℹ 】➜ Transferred ₱${formattedTransferAmount} to ${recipientName}.`);
            } catch (error) {
  ...
  else if (args[0] === 'approve' && userPermissions.includes(2)) {
